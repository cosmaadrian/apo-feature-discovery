...
from dspy.teleprompt.utils import (get_signature, set_signature,
                                   create_minibatch, print_full_program,
...
class MIPROv2(Teleprompter):
    ...
    def compile(
        ...
        **kwargs,
        ...
        if self.auto is not None and (self.num_candidates is not None or num_trials is not None):
            raise ValueError("If auto is not None, num_candidates and num_trials cannot be set, since they would be overridden by the auto settings. Please either set auto to None, or do not specify num_candidates and num_trials.")
        ...
        if os.path.exists(f'checkpoints-mipro/{self.args.experiment_name}/demo_candidates.pkl') and os.path.getsize(f'checkpoints-mipro/{self.args.experiment_name}/demo_candidates.pkl') > 0:
            print("::: Loading demo candidates from disk.")
            with open(f'checkpoints-mipro/{self.args.experiment_name}/demo_candidates.pkl', 'rb') as f:
                demo_candidates = pickle.load(f)
        else:
            demo_candidates = self._bootstrap_fewshot_examples(program, trainset, seed, teacher)
            os.makedirs(f'checkpoints-mipro/{self.args.experiment_name}/', exist_ok = True)
            with open(f'checkpoints-mipro/{self.args.experiment_name}/demo_candidates.pkl', 'wb') as f:
                pickle.dump(demo_candidates, f)

        if os.path.exists(f'checkpoints-mipro/{self.args.experiment_name}/instruction_candidates.pkl') and os.path.getsize(f'checkpoints-mipro/{self.args.experiment_name}/instruction_candidates.pkl') > 0:
            print("::: Loading instruction candidates from disk.")
            with open(f'checkpoints-mipro/{self.args.experiment_name}/instruction_candidates.pkl', 'rb') as f:
                instruction_candidates = pickle.load(f)
        else:
            ...
            os.makedirs(f'checkpoints-mipro/{self.args.experiment_name}/', exist_ok = True)
            with open(f'checkpoints-mipro/{self.args.experiment_name}/instruction_candidates.pkl', 'wb') as f:
                pickle.dump(instruction_candidates, f)
    ...
    def _estimate_lm_calls(
        ...
        prompt_model_line = (f"{YELLOW}- Prompt Generation: {BLUE}{BOLD}10{ENDC}{YELLOW} data summarizer calls + "
        ...
        if not minibatch:
            ...
            task_model_line = (f"{YELLOW}- Program Evaluation: {BLUE}{BOLD}{len(valset)}{ENDC}{YELLOW} examples in val set * "
            ...
            task_model_line = (f"{YELLOW}- Program Evaluation: {BLUE}{BOLD}{minibatch_size}{ENDC}{YELLOW} examples in minibatch * "
                               f"{BLUE}{BOLD}{num_trials}{ENDC}{YELLOW} batches + "
    ...
    def _optimize_prompt_parameters(
        ...
        os.makedirs("./checkpoints-mipro/optuna_studies/", exist_ok = True)
        optuna_study_path = f"./checkpoints-mipro/optuna_studies/{self.args.experiment_name}.log"
